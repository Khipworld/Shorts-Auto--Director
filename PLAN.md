...
**가장 먼저 할 일**: `design/main-screen-mockup.html`을 실제 `StartScreen.tsx`/
`ResultScreen.tsx` 컴포넌트에 반영 (React 상태 연결, 목업의 정적 목값을 실제 파이프라인
데이터로 교체). 그 다음에야 아래 기존 미정 사항(백엔드 영상 렌더링 스타일 불일치, 영상
길이 82초→20-35초 단축 등)으로 넘어갈 것 — 사용자가 화면 구조부터 먼저 확정하길 원했음
(`"프론트앤드 부터 제작"`).
